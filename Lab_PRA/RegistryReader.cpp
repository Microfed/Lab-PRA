...
LAB_PRA_API bool RegistryReader::GetKeysEnum( LPCTSTR lpName, std::vector<tstring>& result ) {
	size_t i = 0;
	...
	if (&currentKeyHandle != nullptr && this->isNoError()) {
		while (retCode == ERROR_SUCCESS) {
			...
			if (retCode == ERROR_SUCCESS) {
				result.push_back(*(new tstring(achKey)));
			}
		}
...
std::basic_string<TCHAR> RegistryReader::ReadString(LPCTSTR subKeyName, LPCTSTR parameterName) {
	...
	if(&currentKeyHandle != NULL && this->isNoError()) {
		...
		if(isNoError()) {
			pcbData = new BYTE[ dwDataSize + sizeof(TCHAR) ];

			if( NULL == pcbData ) {        
				lastError = ERROR_NOT_ENOUGH_MEMORY;
				if( NULL != currentKeyHandle ) {
					closeKey();
				}
			...
			reinterpret_cast<TCHAR*>(pcbData)[ dwDataSize/sizeof(TCHAR) - 1 ] = _T('\0');

			lastError = RegQueryValueEx(currentKeyHandle, parameterName, 0, &dwType, pcbData, &dwDataSize);
			stringValue.append(reinterpret_cast<TCHAR*>(pcbData));
		}
	}
